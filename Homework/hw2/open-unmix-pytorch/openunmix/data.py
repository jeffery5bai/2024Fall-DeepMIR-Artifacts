...
# NOTE: default to use preprocessed NumpyDataset
def load_numpy_datasets(
    parser: argparse.ArgumentParser, args: argparse.Namespace
) -> Tuple[UnmixDataset, UnmixDataset, argparse.Namespace]:
    """Loads numpy dataset from commandline arguments

    Returns:
        train_dataset, validation_dataset
    """
    parser.add_argument("--input-file", type=str, default="mixture.npy")
    parser.add_argument("--output-file", type=str, default="vocals.npy")

    args = parser.parse_args()

    dataset_kwargs = {
        "root": Path(args.np_root),
        "input_file": args.input_file,
        "output_file": args.output_file,
        "seed": args.seed,
    }

    train_dataset = NumpyDataset(split="train", **dataset_kwargs)
    valid_dataset = NumpyDataset(split="valid", **dataset_kwargs)

    return train_dataset, valid_dataset, args


# NOTE: customized dataset to load preprocessed numpy array
class NumpyDataset(UnmixDataset):
    def __init__(
        self,
        root: str,
        split: str = "train",
        input_file: str = "mixture.npy",
        output_file: str = "vocals.npy",
        seed: int = 42,
    ) -> None:
        ...
        The input and output files are numpy arrays after
        preprocessing (random chunk, source augmentation, etc.),
        and transformation (STFT + ComplexNorm encoder).
        ...
        * Source Separation (Mixture -> Target)
        ...
        Example
        =======
        data/train/01/mixture.npy --> input
        data/train/01/vocals.npy ---> output
    ...
    def __getitem__(self, index):
        ...
        X_spectrogram = torch.as_tensor(np.load(input_path), dtype=torch.float32)
        Y_spectrogram = torch.as_tensor(np.load(output_path), dtype=torch.float32)
        # return torch tensors
        return X_spectrogram, Y_spectrogram
