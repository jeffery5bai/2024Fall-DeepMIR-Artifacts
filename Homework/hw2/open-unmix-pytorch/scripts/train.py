...
from openunmix import data, model, transforms, utils
...
def train_on_npy(args, unmix, device, train_sampler, optimizer):
    """no encoder"""
    losses = utils.AverageMeter()
    unmix.train()
    pbar = tqdm.tqdm(train_sampler, disable=args.quiet)
...
def valid_on_npy(args, unmix, device, valid_sampler):
    """no encoder"""
    losses = utils.AverageMeter()
    unmix.eval()
    with torch.no_grad():
...
def main():
    ...
    parser.add_argument("--np-root", type=str, default="../musdb18hq_np", help="root path of numpy dataset")
    parser.add_argument(
        "--output",
        type=str,
        default="open-unmix",
    ...
    musdb_train_dataset, musdb_valid_dataset, args = data.load_datasets(parser, args)
    train_dataset, valid_dataset, args = data.load_numpy_datasets(parser, args)
    
    # create output dir if not exist
    target_path = Path(args.output)
    target_path.mkdir(parents=True, exist_ok=True)

    # NOTE: use NumpyDataset
    train_sampler = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=True, **dataloader_kwargs
    )
    valid_sampler = torch.utils.data.DataLoader(valid_dataset, batch_size=1, **dataloader_kwargs)

    # NOTE: The encoder has been executed in data preprocessing (wav2npy) 
    stft, _ = transforms.make_filterbanks(
        n_fft=args.nfft, n_hop=args.nhop, sample_rate=musdb_train_dataset.sample_rate
    )
    ...
    separator_conf = {
        "nfft": args.nfft,
        "nhop": args.nhop,
        "sample_rate": musdb_train_dataset.sample_rate,
    ...
    if args.checkpoint or args.model or args.debug:
        ...
        scaler_mean, scaler_std = get_statistics(args, encoder, musdb_train_dataset)

    max_bin = utils.bandwidth_to_max_bin(musdb_train_dataset.sample_rate, args.nfft, args.bandwidth)
    ...
    # else start optimizer from scratch
        ...
        t = tqdm.trange(0, args.epochs + 1, disable=args.quiet)
        train_losses = []
        valid_losses = []
        train_times = []
        best_epoch = 0
    ...
    for epoch in t:
        ...
        # NOTE: npy array already been encoded
        ...
        train_loss = train_on_npy(args, unmix, device, train_sampler, optimizer)
        valid_loss = valid_on_npy(args, unmix, device, valid_sampler)
